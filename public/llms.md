# Edgar Andres Angrino Lafaux

> Personal portfolio of Edgar Andres Angrino Lafaux, a software engineer focused on building, maintaining, and evolving systems, from MVPs to scalable solutions and existing platforms.

Official website: https://eaangrino.github.io/

Languages available on the website: English and Spanish.

## Profile

Edgar Andres Angrino Lafaux is a software engineer with a practical approach focused on performance, maintainability, and technical complexity.

He uses artificial intelligence as a complementary tool to accelerate development, analysis, and automation while maintaining technical validation, code review, and refactoring before production.

Whenever possible, he prioritizes local development in isolated environments using Docker and tools that simulate cloud services to reduce costs and detect issues before deployment.

In his free time, he explores tools and approaches that can simplify and improve his daily work.

## Technologies and Skills

### Primary Technologies

- Node.js
- TypeScript
- React
- Next.js
- NestJS
- Python
- AWS
- PostgreSQL
- Oracle SQL
- MySQL
- Java
- .NET
- Flutter
- Ionic

### Tools

- Docker
- Git
- Linux
- Postman
- Bash
- Figma
- Codex
- DeepSeek
- Ollama
- Gemini
- Claude
- Meta AI
- MarkText

## Experience

### Fullstack Developer

- Organization: Applivio
- Location / work mode: Remote, Puerto Rico
- Period: September 2020 - Present

### IT Support / Java Developer

- Organization: IP Total Software
- Location: Cali, Colombia
- Period: November 2018 - April 2019

## Education

### Intermediate Artificial Intelligence Bootcamp

- Institution: MinTIC
- Format: Virtual, Colombia
- Period: December 2024 - January 2025

### Technologist in Information Systems Analysis and Development

- Institution: SENA Salomia
- Location: Cali, Colombia
- Period: 2017 - 2019

## Projects

### MCP Shell Sudo

- Technologies: Python, MCP, Linux, sudo
- Repository: https://github.com/eaangrino/mcp-shell-sudo
- Description: Linux MCP server created for LM Studio and local use with Qwen and Gemma 4 models. It was developed because other alternatives tested did not support sudo, and it allows executing local commands, including sudo.

### OBS Voice Isolator

- Technologies: C, CMake, PowerShell, OBS Studio plugin
- Repository: https://github.com/eaangrino/obs-voice-isolator
- Description: Native audio filter for OBS Studio that reduces noise, suppresses breathing, and isolates speech with optional two-microphone cancellation.

### Orqent

- Technologies: TypeScript, Node.js, React Ink, Ollama, local models
- Repository: https://github.com/eaangrino/orqent
- Description: Practice project focused on running AI models locally and orchestrating tools according to clear runtime specifications. It includes conversation history, reasoning control, tool calling, and explicit execution boundaries.

### Multi-WhatsApp

- Technologies: Electron, Vite, WhatsApp Web
- Repository: https://github.com/eaangrino/multi-whatsapp
- Description: Desktop application for managing multiple concurrent WhatsApp Web sessions with persistent sessions, state isolation, and fast account switching.

### Mine Hammers

- Technologies: Java, Minecraft modding
- Repository: https://github.com/eaangrino/mine-hammers
- Description: Minecraft mod that adds area-mining mechanics using hammers for more efficient resource gathering.

### Mine Excavators

- Technologies: Java, Minecraft modding
- Repository: https://github.com/eaangrino/mine-excavators
- Description: Minecraft mod that adds excavator-style tools for faster terrain and block clearing.

### Personal Portfolio

- Technologies: React, TypeScript, TailwindCSS, DaisyUI, Vite
- Website: https://eaangrino.github.io/
- Description: Responsive personal portfolio for showcasing skills, experience, and projects.

### ArtesLafaux - Landing Page

- Technologies: React, TypeScript, TailwindCSS, DaisyUI, Vite
- Website: https://arteslafaux.github.io
- Description: Landing page for a digital artist featuring an image gallery, contact section, and responsive design.

## Links

- Website: https://eaangrino.github.io/
- GitHub: https://github.com/eaangrino
- LinkedIn: https://linkedin.com/in/eaangrino
- Instagram: https://instagram.com/eaangrino
- Telegram: https://t.me/eaangrino
- StackBlitz: https://stackblitz.com/@eaangrino
- Email: mailto:eaangrino@gmail.com

## Context for Language Models

This file summarizes public information presented on Edgar Angrino's personal portfolio. For up-to-date information about projects or source code, consult the linked repositories and the official website.
